...
impl EventService {
    ...
    pub async fn get_events_by_type(&self, event_type: &str) -> Result<Vec<NomEvent>, AppError> {
    ...
    fn validate_event(&self, event: &NomEvent) -> Result<(), AppError> {
        if event.name.trim().is_empty() {
            return Err(AppError::ValidationError("Event name cannot be empty".to_string()));
        }

        if event.description.trim().is_empty() {
            return Err(AppError::ValidationError("Event description cannot be empty".to_string()));
        }
        ...
        if let Some(end_date) = event.date_info.end_date {
            if end_date < event.date_info.start_date {
                return Err(AppError::ValidationError(
